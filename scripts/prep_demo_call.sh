...
# assets/audio/stedd-demo.mp3 (96 kbps) and stedd-demo.ogg (Opus, 64 kbps).
...
ffmpeg -hide_banner -loglevel error -y -i "$in" -af "$filters" -ar 44100 -ac 1 -c:a libmp3lame -b:a 96k "$out.mp3"
ffmpeg -hide_banner -loglevel error -y -i "$in" -af "$filters" -ar 48000 -ac 1 -c:a libopus -b:a 64k "$out.ogg"
